...
@Component({
  ...
  templateUrl: './aggrereqadmin.component.html',
  styleUrls: ['./aggrereqadmin.component.css']
})
export class AggrereqadminComponent implements OnInit {
    ...
    for(var m in this.cart)
      {
        if(this.cart[m]>0)
        {
          ...
          this.db.collection("pickup_aggre").add(req).then(res => {
