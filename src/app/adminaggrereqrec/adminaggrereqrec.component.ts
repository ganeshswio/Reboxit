...
@Component({
  ...
  templateUrl: './adminaggrereqrec.component.html',
  styleUrls: ['./adminaggrereqrec.component.css']
})
export class AdminaggrereqrecComponent implements OnInit {
  ...
  constructor(private db:AngularFirestore,private router:Router,private modalService: NgbModal,private auth:AuthService) { 
    this.db.collection("pickup_aggre",ref => ref.where("assigned","==",false)).snapshotChanges().subscribe(res => {
      for(var i in res)
      {
  ...
  submit(idx)
  {
    if(this.assSelect[idx])
    {
      ...
      this.db.collection("pickup_aggre").doc(this.pickup[idx]["id"]).update(this.pickup[idx]).then(res =>{console.log("yeah");
